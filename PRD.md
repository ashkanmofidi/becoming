...
If timer is running when user logs out: [CHANGED v3.1.9 - 2026-04-09] Do NOT log out immediately. Show confirmation modal: "You have a focus session in progress. Logging out will reset your timer. Continue?" with Cancel (stays on page, timer intact) and Log Out (abandons session, writes partial data to logs, clears timer, redirects to login). Modal includes "Don't ask me again" checkbox — when checked and confirmed, saves preference to user settings. Future logouts skip the modal. [ADDED v3.1.10 - 2026-04-09] Preference resettable via Settings > Behavior > "Ask before logout with active timer" toggle. Same behavior for 3-day session expiry with active timer — show warning before forcing logout.
3.4 Account View
...
CONTRADICTION FIX: Current screenshots show timer at "00:00" in idle. This is a BUG. Idle state MUST show the configured duration (e.g., "25:00"). The spec is authoritative; the implementation must be fixed. [FIXED v3.1.1 - 2026-04-09] [UPDATED v3.1.12 - 2026-04-09] Timer page must NOT render any duration until user settings are loaded. Show a loading skeleton instead. The first visible frame must show the user's saved focus duration, never a hardcoded default. Timer display initializes to configured focus duration (25:00 default) before server state loads. Null server state (first-time user) also defaults to 25:00.
...
AUDIO LIFECYCLE [CHANGED v3.1.14 - 2026-04-09]: Tick sounds managed by lib/tick-engine.ts — a pure singleton module with ZERO React dependency. Uses Web Audio API scheduled timing (audioContext.currentTime, Chris Wilson pattern) for sample-accurate 1-second intervals. Two methods: startTick() and stopTick(), called from timer event handlers. No component lifecycle, no navigation listeners, no polling. Survives all navigation inherently because it's not a component.
LOGIN TIMER RESET [ADDED v3.1.14 - 2026-04-09]: On every fresh login, timer state is cleared from KV. User always lands on focus timer, never on a stale break timer from a previous session. It must NOT be destroyed on component unmount — AudioContext requires a user gesture to re-create, and destroying it kills tick sounds when the user navigates to Settings and back. The engine is initialized once on the first user interaction and lives until the browser tab closes.
...
MUTE CONTROL [CHANGED v3.1.15 - 2026-04-09]: Single `muted` boolean in UserSettings, persisted to KV. Both the timer page mute button and the Settings page "Mute All Sounds" toggle read/write the same field. Last write wins. No localStorage, no component state. On login, saved mute state loads everywhere. All audio routes through master gain node — mute sets gain to 0. Mute before pressing Start = zero audio including start chime, ticks, and completion. Ambient sound stops on mute, resumes on unmute if session is active.
